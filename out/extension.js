        ...
        if (e.affectsConfiguration('vscodeBackground')) {
            ...
            if (enabled && videoFiles.length > 0) {
                ...
                if (e.affectsConfiguration('vscodeBackground.opacity') ||
                    ...
                    try {
                        await applyVideoBackground(videoFiles);
                        vscode.window.showInformationMessage('Background settings updated! Please restart VSCode to see changes.', 'Restart').then(selection => {
                            if (selection === 'Restart') {
                                vscode.commands.executeCommand('workbench.action.reloadWindow');
                            }
                        });
...
`;
        ...
        const action = await vscode.window.showInformationMessage('This will open an Administrator prompt to grant file permissions. Continue?', 'Grant Permissions', 'Cancel');
    ...
    const generateInstallScript = vscode.commands.registerCommand('vscode-background.generateInstallScript', async () => {
        ...
        if (saveUri) {
            ...
            vscode.window.showInformationMessage('Script saved! Right-click and select "Run with PowerShell" as Administrator.', 'Open Folder').then(action => {
                if (action === 'Open Folder') {
                    (0, child_process_1.exec)('explorer /select,"' + saveUri.fsPath + '"');
                }
            });
        }
    });
    ...
    context.subscriptions.push(enableBackground, disableBackground, configureBackground, diagnostics, configWatcher, addVideos, removeVideo, manageVideos, fixChecksums, setInfiniteLoop, grantPermissions, generateInstallScript, cleanup);
}
