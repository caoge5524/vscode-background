...
class Background {
    ...
    async install() {
        ...
        if (config.videos.length === 0) {
            const action = await vscode.window.showWarningMessage('未配置媒体文件（视频/图片）。请先在 settings.json 的 "vscodeBackground.videos" 中添加文件路径，或使用"添加视频"命令。', '添加媒体', '编辑 settings.json', '打开设置', '取消');
            if (action === '添加媒体') {
                await this.addVideos();
            }
        ...
        if (missingFiles.length > 0) {
            const action = await vscode.window.showWarningMessage(`以下 ${missingFiles.length} 个媒体文件不存在:\n${missingFiles.map(f => path.basename(f)).join(', ')}\n\n是否仍然继续？`, '继续', '编辑 settings.json', '取消');
        ...
        try {
            ...
            const action = await vscode.window.showInformationMessage('✅ 背景已应用！请重启 VSCode 以查看效果。', '立即重启');
            if (action === '立即重启') {
    ...
    async addVideos() {
        ...
        const action = await vscode.window.showInformationMessage(`已添加 ${selectedFiles.length} 个媒体文件: ${names}`, '立即应用', '编辑 settings.json', '稍后');
        if (action === '立即应用') {
            await this.install();
        }
        else if (action === '编辑 settings.json') {
    ...
    async openWorkshop() {
        ...
        const action = await vscode.window.showInformationMessage('背景创意工坊：与社区一起分享和发现精美背景！', '打开 Discussions', '查看工坊指南');
